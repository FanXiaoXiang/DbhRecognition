...
import com.bjfu.dbhrecognition.engine.encode.EncodeLibBaseRGB;
...
public class DecodeBaseRGB extends Decode {
    private static volatile DecodeBaseRGB decode;
    private int samplePlots = 2;

    private DecodeBaseRGB() {

    }

    public static DecodeBaseRGB getInstance() {
        if (decode == null) {
            synchronized (DecodeBaseRGB.class) {
                if (decode == null) {
                    decode = new DecodeBaseRGB();
                }
            }
        }
        return decode;
    ...
    public String getIntegerPart(Bitmap bm, Point start, Point end, ColorRecognition recognition) {
        ...
        if (start.x == end.x) {
            step = (end.y - start.y) * 1.0 / ((EncodeLibBaseRGB.INT_PART_MODELS - 1) * 1.0);
        } else {
            step = (end.x - start.x) * 1.0 / ((EncodeLibBaseRGB.INT_PART_MODELS - 1) * 1.0);
            k = (end.y - start.y) * 1.0 / (end.x - start.x) * 1.0;
            b = start.y - k * start.x;
        }
        //获取每个模块对应的颜色类型，并获取值，添加到buffer中
        StringBuffer buffer = new StringBuffer(EncodeLibBaseRGB.INT_PART_MODELS);
        Color color = new Color();
        for (int i = 0; i < EncodeLibBaseRGB.INT_PART_MODELS; i++) {
            if (i == 0 || i == EncodeLibBaseRGB.INT_PART_MODELS - 1) {
        ...
        if (code.length() != EncodeLibBaseRGB.INT_PART_MODELS) return null;
        if (!code.endsWith(EncodeLibBaseRGB.ENCODE_END))
            code = buffer.reverse().toString().trim();
        //若是该编码类型，截取数据区，然后解码两位数字位，并放入buffer中，因为解码失败返回-1，所以当buffer长度
        //为2时解码正确，此时返回相应值
        code = code.substring(2, 8);
        buffer.delete(0, buffer.length());
        buffer.append(EncodeLibBaseRGB.getValue(code.substring(0, 3)));
        buffer.append(EncodeLibBaseRGB.getValue(code.substring(3, 6)));
    ...
    public String getDecimalPart(Bitmap bm, Point start, Point end, ColorRecognition recognition) {
        ...
        if (start.x == end.x) {
            step = (end.y - start.y) * 1.0 / ((EncodeLibBaseRGB.DECIMAL_PART_MODELS - 1) * 1.0);
        } else {
            step = (end.x - start.x) * 1.0 / ((EncodeLibBaseRGB.DECIMAL_PART_MODELS - 1) * 1.0);
            k = (end.y - start.y) * 1.0 / (end.x - start.x) * 1.0;
            b = start.y - k * start.x;
        }
        //获取每个模块对应的颜色类型，并获取值，添加到buffer中
        StringBuffer buffer = new StringBuffer(EncodeLibBaseRGB.DECIMAL_PART_MODELS);
        Color color = new Color();
        for (int i = 0; i < EncodeLibBaseRGB.DECIMAL_PART_MODELS; i++) {
            if (i == 0 || i == EncodeLibBaseRGB.DECIMAL_PART_MODELS - 1) {
                buffer.append(ColorType.BLACK.getValue());
            } else {
                if (start.x == end.x) {
                    getSamplePlotsColor(bm, start.x, (int) (start.y + i * step), samplePlots, color);
        ...
        //根据初始符、终止符判断是否为该编码类型，若不是该编码类型则返回null
        String code = buffer.toString().trim();
        if (code.length() != EncodeLibBaseRGB.DECIMAL_PART_MODELS) return null;
        if (!code.endsWith(EncodeLibBaseRGB.ENCODE_END))
            code = buffer.reverse().toString().trim();
        //截取数据区，解码，并将数据放入buffer中，若长度为1，说明解码正确，返回相应的小数值
        code = code.substring(2, 5);
        buffer.delete(0, buffer.length());
        buffer.append(EncodeLibBaseRGB.getValue(code));
